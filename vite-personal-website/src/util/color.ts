  ...
  export class Color {
    ...
    constructor(value: string | RGB | HSL) {
      if (typeof value === 'string' && /rgb\(/.test(value)) {
        ...
        value = !matches ? "" : { r: parseInt(matches[1], 10), g: parseInt(matches[2], 10), b: parseInt(matches[3], 10) };
      } else if (typeof value === 'string' && /hsl\(/.test(value)) {
        const matches = /hsl\((\d{1,3}), ?(\d{1,3}%), ?(\d{1,3}%)\)/.exec(value) ?? [];
        value = !matches ? "" : { h: parseInt(matches[1], 10), s: parseInt(matches[2], 10), l: parseInt(matches[3], 10) };
      }
    ...
    static isColor(value: string): boolean {
      ...
      const matches = value.trim().match(/(^#[0-9a-fA-F]+)/);
      return (matches != null && value.trim() === matches[0]);
    }
